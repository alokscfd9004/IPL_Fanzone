...
class OfflineAnswerTests(TestCase):
    """Offline-first: verified-dataset answers need no Groq quota/key"""

    def setUp(self):
        self.client = Client()

    def _ask(self, message):
        resp = self.client.post('/jarvis/ask/', data=json.dumps({'message': message}),
                                content_type='application/json')
        self.assertEqual(resp.status_code, 200)
        return json.loads(resp.content)

    def test_season_champion_offline(self):
        data = self._ask('who won IPL 2016?')
        self.assertEqual(data.get('source'), 'offline')
        self.assertIn('Sunrisers', data['response'])

    def test_orange_cap_offline(self):
        data = self._ask('orange cap 2023')
        self.assertEqual(data.get('source'), 'offline')
        self.assertIn('Gill', data['response'])

    def test_champions_list_offline(self):
        data = self._ask('list all IPL champions')
        self.assertEqual(data.get('source'), 'offline')
        self.assertIn('2008', data['response'])

    @patch('apps.ai_bot.views.HAS_GROQ', False)
    def test_offline_works_without_api_key(self):
        data = self._ask('who won IPL 2016?')
        self.assertEqual(data.get('source'), 'offline')
        self.assertIn('Sunrisers', data['response'])

    @patch('apps.ai_bot.views.HAS_GROQ', False)
    def test_open_question_reports_missing_key(self):
        data = self._ask('predict the next IPL winner')
        self.assertEqual(data.get('source'), 'no-key')

    @patch('apps.ai_bot.views.HAS_GROQ', True)
    @patch('apps.ai_bot.views.client')
    def test_open_question_falls_through_to_groq(self, mock_client):
        resp_obj = MagicMock()
        resp_obj.choices = [MagicMock(message=MagicMock(content='AI prediction!'))]
        mock_client.chat.completions.create.return_value = resp_obj
        data = self._ask('predict the next IPL winner')
        self.assertEqual(data.get('source'), 'groq')
        self.assertEqual(data['response'], 'AI prediction!')

    @patch('apps.ai_bot.views.HAS_GROQ', True)
    @patch('apps.ai_bot.views.client')
    def test_offline_answers_do_not_consume_ai_quota(self, mock_client):
        resp_obj = MagicMock()
        resp_obj.choices = [MagicMock(message=MagicMock(content='ok'))]
        mock_client.chat.completions.create.return_value = resp_obj
        for _ in range(15):
            data = self._ask('who won IPL 2016?')
            self.assertEqual(data.get('source'), 'offline')
        # 15 offline asks must NOT hit the 10/min Groq limit for a later AI call
        data = self._ask('predict the next IPL winner')
        self.assertEqual(data.get('source'), 'groq')


class RateLimitTests(TestCase):
    """Per-user + global Groq quota protection (10/min, 50/day, 1000/day global)"""

    def _mock_ai(self, mock_client):
        resp_obj = MagicMock()
        resp_obj.choices = [MagicMock(message=MagicMock(content='ok'))]
        mock_client.chat.completions.create.return_value = resp_obj
    ...
    @patch('apps.ai_bot.views.HAS_GROQ', True)
    @patch('apps.ai_bot.views.client')
    def test_user_minute_limit_blocks_11th_call(self, mock_client):
        self._mock_ai(mock_client)
        c = Client()
        codes = [c.post('/jarvis/ask/', data=json.dumps({'message': 'predict IPL winner'}),
                        content_type='application/json').status_code for _ in range(12)]
        self.assertEqual(codes[:10], [200] * 10)
        self.assertEqual(codes[10:], [429, 429])

    @patch('apps.ai_bot.views.HAS_GROQ', True)
    @patch('apps.ai_bot.views.client')
    def test_429_body_has_retry_info(self, mock_client):
        self._mock_ai(mock_client)
        c = Client()
        for _ in range(11):
            resp = c.post('/jarvis/ask/', data=json.dumps({'message': 'predict IPL winner'}),
                          content_type='application/json')
    ...
    @patch('apps.ai_bot.views.HAS_GROQ', True)
    @patch('apps.ai_bot.views.client')
    def test_other_user_not_affected(self, mock_client):
        self._mock_ai(mock_client)
        heavy, light = Client(), Client()
        for _ in range(10):
            heavy.post('/jarvis/ask/', data=json.dumps({'message': 'predict IPL winner'}),
                       content_type='application/json')
        resp = light.post('/jarvis/ask/', data=json.dumps({'message': 'predict IPL winner'}),
                          content_type='application/json')
        self.assertEqual(resp.status_code, 200)

    def test_live_api_rate_limited_per_user(self):
        c = Client()
        codes = [c.get('/scorecard/api/live/').status_code for _ in range(35)]
        self.assertEqual(codes[:30], [200] * 30)
        self.assertIn(429, codes[30:])
